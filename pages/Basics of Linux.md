- #Commands
	...
	- **`ssh` (OpenSSH client)**
		- **Definition**:
			- A secure remote login and command execution tool that connects to a remote host over an encrypted SSH protocol (typically TCP port 22).
		- **Basic Functions**:
			- Remote shell access.
			- Run a single command remotely (non-interactive).
			- Port forwarding (local/remote/dynamic).
			- File transfer (via `scp`/`sftp`) and agent-based authentication.
		- **Core Syntax**:
			- ```
		...
		- **Common Options**:
			- **Specify port**: `-p 2222`
			- **Identity (private key) file**: `-i ~/.ssh/id_ed25519`
			- **Verbose debug output**: `-v` / `-vv` / `-vvv`
			- **Do not run remote command** (just forward ports): `-N`
			- **Do not allocate a TTY** (useful for scripts): `-T`
			- **Force TTY allocation**: `-t`
			- **Enable compression**: `-C`
			- **Connection timeout**: `-o ConnectTimeout=5`
			- **Disable strict host key checking** (use with care): `-o StrictHostKeyChecking=no`
			- **Use a specific config file**: `-F ~/.ssh/config`
		- **Authentication & Keys (Quick Notes)**:
			- Keys are usually stored in `~/.ssh/`:
			...
			- Add your public key to the server:
			  ...
			  *(if `ssh-copy-id` is not available, you can manually append to `~/.ssh/authorized_keys` on the server).*
		- **SSH Config (`~/.ssh/config`)**:
			- Helps create shortcuts and store per-host settings:
			...
			- Then connect with:
			  ...
			  ```
		- **Port Forwarding**:
			- **Local forwarding** (expose remote service locally):
			  
			  ```
			  ssh -L 8080:localhost:80 user@host
			  ```
			- **Remote forwarding** (expose local service on remote side):
			...
			- **Dynamic forwarding** (SOCKS proxy):
			  ...
			  ```
		- **Notes / Gotchas**:
			- Use `ssh -v` to debug most connection/auth issues (DNS, routing, host keys, key permissions, etc.).
			- If you see “WARNING: REMOTE HOST IDENTIFICATION HAS CHANGED!”, you’re hitting a host key mismatch (could be legitimate reinstall or a security issue).
		...
		- **Examples**:
			- **Connect with a username**:
			  ...
			  ```
			- **Connect on a custom port with a specific key**:
	...
	- **`.bashrc`**
		- **Definition**:
			- A shell startup script executed by **Bash** for **interactive non-login shells** (typically when you open a new terminal window/tab on many Linux distros).
		- **Basic Functions**:
		...
		- **When It Runs (Important)**:
			- Runs for **interactive** Bash shells that are **not login shells**.
			- A **login shell** typically reads `~/.bash_profile` (or `~/.profile`) instead.
			- Many distros make `~/.bash_profile` source `~/.bashrc` so settings apply everywhere:
		...
		- **Notes / Gotchas**:
			...
			- For scripts, don’t rely on `.bashrc` (non-interactive shells may not load it).
			- On macOS, the default shell is often `zsh`, so `.zshrc` is the equivalent (unless you explicitly use bash).
			- Keep `.bashrc` fast: avoid slow commands that run every time a shell opens.
		- **Examples**:
			- **Quick reload after editing**:
